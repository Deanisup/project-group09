...
session_id integer NOT null, 
animal_id integer NOT NULL,
activity_code integer DEFAULT 0,
when_measured timestamp NOT NULL,
PRIMARY key(session_id, animal_id, activity_code, when_measured),
...
CREATE VIEW Goat_Rankings AS
SELECT DISTINCT g.animal_id, g.total_points, v.activity_code
FROM Goat_Points AS g
LEFT JOIN VaxedGoats AS v
ON g.animal_id=v.animal_id 
ORDER BY Total_points DESC;

DROP VIEW Family_Tree;
CREATE VIEW Family_Tree AS
SELECT animal_id, sire, dam
FROM Goat;

DROP VIEW VaxAverages;
CREATE VIEW VaxAverages AS
SELECT AVG(total_points), chlamydia_vaccination
FROM Goat_Points AS g, Trait AS t
WHERE t.animal_id=g.animal_id
GROUP BY Chlamydia_vaccination;


DROP VIEW Goat_Health;
CREATE VIEW Goat_Health AS
SELECT G.*, GP.total_points
FROM Goat G
LEFT JOIN Goat_Points GP ON G.animal_id = GP.animal_id;
